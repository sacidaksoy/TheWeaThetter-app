...
import { SortAscIcon, CheckIcon, Globe, XCircle, Globe2 } from "lucide-react";
...
export function CityPickerSidebar() {
      ...
      {selectedCountry &&
        (City.getCitiesOfCountry(selectedCountry.isoCode)?.map((item) => item)
          ...
          <Popover open={openCity} onOpenChange={setOpenCity}>
            <Tooltip label={selectedCity?.name ?? "City"} side="right" asChild>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  role="combobox"
          ...
          <Tooltip label={"No City Found"} side="right" asChild>
            <div
              className={cn(
                "flex items-center space-x-2 ml-6",
                !collapsed ? "ml-4" : "ml-6"
              )}
            >
              <XCircle className="h-5 w-5 text-red-500" />
              <Label htmlFor="city" className="text-sm">
                {!collapsed ? "No City Found" : ""}
              </Label>
            </div>
          </Tooltip>
        ))}
